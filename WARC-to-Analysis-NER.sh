...
stanford-ner/ner.sh fulltext.txt > fulltext_ner.txt
sed 's/\/O / /g' < fulltext_ner.txt > fulltext_ner_clean.txt
    
# extracting persons and counting them
egrep -o -f personpattr fulltext_ner_clean.txt > fulltext_ner_pers.txt
cat fulltext_ner_pers.txt | sed 's/\/PERSON//g' | sort | uniq -c | sort -nr > fulltext_ner_pers_freq.txt
    
# extracting organizations and counting them
egrep -o -f orgpattr fulltext_ner_clean.txt > fulltext_ner_org.txt
cat fulltext_ner_org.txt | sed 's/\/ORGANIZATION//g' | sort | uniq -c | sort -nr > fulltext_ner_org_freq.txt
    
# extracting locations and counting them
egrep -o -f locpattr fulltext_ner_clean.txt > fulltext_ner_loc.txt
cat fulltext_ner_loc.txt | sed 's/\/LOCATION//g' | sort | uniq -c | sort -nr > fulltext_ner_loc_freq.txt

# now run Mathematica Script

alias math="/Applications/Mathematica.app/Contents/MacOS/MathKernel"

math -script WARC-to-Analysis-Mathematica-ner.m $KWIC